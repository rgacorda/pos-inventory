...
/**
 * Round an amount up to the next whole peso.
 * Whole amounts (e.g. 50.00) are unchanged. Zero/negative stay 0.
 */
export function roundUpToWholePeso(amount: number): number {
  const normalized = Number(Math.max(0, amount).toFixed(2));
  return Math.ceil(normalized);
}

/**
 * Apply optional whole-peso round-up to a price or amount due.
 */
